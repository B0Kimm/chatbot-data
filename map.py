...
#             latitude = float(request.GET.get('longitude', None))
#             position = (latitude, longitude)
#             condition = Q(latitude__range = (latitude - 0.01, latitude +0.01))|
...
#     imsi = [row['shopid'], row['nickname'],row['reviewid'],row['time'], row['menu_summary'], row['comment'] ]
#     result.append(imsi)    

# result = pd.DataFrame(result, columns=column1s)
# result.to_csv('result.csv', mode='w', encoding='utf-8', index=False)
...
for index, row in myoutcome.iterrows():
    if row['구'] == '강남구' :
        ...
        imsi = [row['shopid'], row['name'],row['lat'],row['lng'], row['address'],row['구'], row['동'], row['지번'] ]
