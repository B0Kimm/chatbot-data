...
from utils.test import is_number
...
class OrderReview :
    ...
    def hook_process(self) :
        print('==== HOOK =====')
        self.train()
        print(self.classify('완전별로'))

    def load_corpus(self) :
        reader = self.reader
        corpus = read_table('./data/review.txt', sep=',', encoding='UTF-8')
        # print(f'corpus Spec : {corpus}')
        return np.array(corpus)

    def count_words(self, training_set) :
        counts = defaultdict(lambda: [0,0])
        for review, point in training_set:
            # 영화리뷰가 text일때만 카운팅
            if is_number(review) is False :
                words = review.split()
                for word in words:
    ...
    def class0_probabilities(self, word_probs, review) :
    ...
    def classify(self, review) :
        return self.class0_probabilities(self.word_probs, review)
